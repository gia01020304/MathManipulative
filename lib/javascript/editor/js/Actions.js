			...
			this.editorUi.pickColor($("#pen-tool").attr("color"), function(newColor) {
				function checkPositionValid(x, y) {
					...
					backgroundOffset.right =
						backgroundOffset.left + divBackGround.width();
					backgroundOffset.bottom =
						backgroundOffset.top + divBackGround.height();
				...
				$("#geDiagramContainer svg, #geDiagramContainer svg *").css(
					"cursor",
					"url(" + img.src + ")1 50,crosshair"
				);

				line = d3.line().curve(d3.curveBasis);
				var closedPath = false;
				var svg = d3.select("#geDiagramContainer svg").call(
					d3
						.drag()
						.container(function() {
							return this;
				...
				function dragstarted() {
					if (drawFree && checkPositionValid(event.clientX, event.clientY)) {
						...
						d3.event.on("drag", function() {
							if (
								drawFree &&
								checkPositionValid(event.clientX, event.clientY)
							) {
								var x1 = d3.event.x,
									y1 = d3.event.y,
									dx = x1 - x0,
		...
		elt => {
			...
			try {
				...
				var symbol2 = new mxCell(
					...
					"editable=0;resizable=0;aspect=fixed;rotationEnabled=0;constituent=1;"
				);
				symbol2.value = "X";
...
function processTransferRuler(
	...
	try {
		...
		var symbol2 = new mxCell(
			...
			"editable=0;resizable=0;aspect=fixed;rotationEnabled=0;"
		);
		symbol2.value = "X";
		symbol2.identity = "hide-ruler";
		symbol2.connectable = false;
