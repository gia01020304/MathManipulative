...
this.addAction(
	...
	// 		// ui.freehand.setSmoothing(1);
	// 		// ui.freehand.setEnabled(true);
	// 	},
	// 	// mxUtils.bind(this, function() {
	// 	// 	if (this.customWindow == null) {
	// 	// 		// LATER: Check layers window for initial placement
	// 	// 		this.customWindow = new CustomWindow(
	// 	// 			ui,
	...
	// 	// 			80
	// 	// 			);
	// 	// 		this.customWindow.window.setVisible(true);
	// 	// 		ui.fireEvent(new mxEventObject("customWindow"));
	// 	// 		


	// 	// 	} else {
	// 	// 		this.customWindow.window.setVisible(
	// 	// 			!this.customWindow.window.isVisible()
	// 	// 			);
	// 	// 	}
	// 	// }),
	// 	null,
	...
	this.addAction(
		...
		elt => {
			graph.getModel().beginUpdate();
			try {
				var allPen=graph.getChildVertices(graph.getDefaultParent()).filter(x=>x.class=="pen-tool");
				graph.removeCells(allPen);
			} finally {
	...
	this.addAction(
		"pen-tool",
		elt => {
			var t=this.editorUi.freeHand;
			this.editorUi.pickColor($("#pen-tool").attr("color"), function(newColor) {
				graph.currentVertexStyle[mxConstants.STYLE_STROKEWIDTH]=2;
				graph.currentVertexStyle[mxConstants.STYLE_STROKECOLOR] =newColor;
				t.setSmoothing(1);
				t.startDrawing();
			})
			...
			// this.editorUi.pickColor($("#pen-tool").attr("color"), function(newColor) {
			// 	$("#geDiagramContainer svg").css("cursor", "crosshair");
			...
			// 	graph.getModel().beginUpdate();
			// 	try {
			// 		var rubber=graph.getRubberband();
			...
			// 	function dragged() {
			// 		if (drawFree) {
			// 			var last=d3.mouse(this);
			// 			activeLine.datum().push(last);
			...
			// 	function dragended() {
			// 		if (activeLine!=null) {
			// 			var drawPoints=activeLine.datum();
			...
			// 				var cell = new mxCell('', new mxGeometry(minX / s - tr.x, minY / s - tr.y, w / s, h / s), style+";strokeColor="+newColor+";strokeWidth=2;rotatable=0;editable=0;");
			// 				cell.vertex = 1;
			// 				cell.class="pen-tool";
...
this.addAction(
	...
	elt => {
		...
		try {
			...
			$("#geDiagramContainer svg").css("cursor", "default");
			graph.graphHandler.setSelectEnabled(true);
			graph.graphHandler.setMoveEnabled(true);
...
this.addAction(
	"text-tool",
	elt => {
		var src = pathRelative + "/lib/images/text.png";
		$("#geDiagramContainer svg, #geDiagramContainer svg *").css(
			"cursor",
			"url(" + src + ")1 50,crosshair"
			);	
		textAble = true;
		drawFree=false;
		graph.getModel().beginUpdate();
...
this.addAction(
	...
	this.addAction(
		...
		elt => {
			var cell = null;
			if (
				typeof graph.idProtractor !== "undefined" &&
				graph.idProtractor !== null
				) {
		...
		try {
			...
			var cell = graph.insertVertex(
				...
				400,
				275,
				"rotatable=0;editable=0;resizable=0;aspect=fixed;shape=protractor"
				);
			cell.identity = "protractor";
			var symbol2 = new mxCell(
				"",
				new mxGeometry(0.95, 0, 20, 20),
				"editable=0;resizable=0;aspect=fixed;rotatable=0;constituent=1;strokeColor=#002FA7;fontColor=#002FA7"
				);
			symbol2.value = "X";
	...
	this.addAction("showerror", elt => {}, null, null, "showerror");
	this.addAction(
		"new-file",
		elt => {
...
function processTransferRuler(
	...
	try {
		if (
			typeof graph.idRuler !== "undefined" &&
			graph.idRuler !== null &&
			oldObject == true
			) {
	...
	var cell = graph.insertVertex(
		...
		width + 40 + 20,
		100,
		"rotatable=0;editable=0;resizable=0;aspect=fixed;shape=" + typeRuler
		);
	cell.identity = "ruler";
	cell.setConnectable(false);
	var symbol2 = new mxCell(
		"",
		new mxGeometry(0.95, 0.1, 20, 20),
		"editable=0;resizable=0;aspect=fixed;rotatable=0;strokeColor=#002FA7;fontColor=#002FA7"
		);
	symbol2.value = "X";
	symbol2.identity = "hide-ruler";
	symbol2.connectable = false;
...
function processProtractor(graph, nameShape, width, height, x, y) {
	...
	try {
		...
		var v11 = graph.insertVertex(
			...
			0,
			0,
			"id=temp;portConstraint=northsouth;",
			true
			);
		...
		graph.insertEdge(
			...
			v1.getChildAt(0),
			number,
			"startArrow=oval;strokeWidth=4;strokeColor=#69b630;resizable=0;"
			);
	} catch (e) {
	} finally {
		graph.getModel().endUpdate();
