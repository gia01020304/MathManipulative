 ...
 */
 (function()
 {
 	...
 	Protractor.prototype.paintVertexShape= function(c, x, y, w, h){
 		...
 		c.moveTo(x6,y6);
 		var x7,y7,x8,y8;
 		x7=r+(r2-3)*findX(radi-1);
 		y7=r-(r2-3)*findY(radi-1);
 		x8=r+(r2-3)*findX(radi+1);
 		y8=r-(r2-3)*findY(radi+1);
 		c.lineTo(x7,y7);
 		c.lineTo(x8,y8);
 		c.lineTo(x6,y6);
 		c.moveTo(x6,y6);
 		...
 		c.setStrokeColor('black');
 		c.moveTo(w,h);
 		c.rect(w-30,h+10,30,30);
 		var value=Math.round(radi<0?radi*-1:radi);
 		c.text(w-15,h+20,0,0,value.toString(),'center');
 		c.stroke();
 	...
 	NumberLine.prototype.paintVertexShape = function(c, x, y, w, h){
 		...
 		var fh=h;
 		if (NumberLine.prototype.cst.Minimum>NumberLine.prototype.cst.Maximum) {
 			return;
 		}
 		var partWidth=w/30;
 		var end=w-2*partWidth;
 		...
 		c.moveTo(partWidth,h/2);
 		c.lineTo(w-partWidth,h/2);
 		c.stroke();
 		c.end();
 		...
 		var total=Math.round((NumberLine.prototype.cst.Maximum-NumberLine.prototype.cst.Minimum)/NumberLine.prototype.cst.Interval);	
 		var value=NumberLine.prototype.cst.Minimum;
 		if (total>10) {
 			this.state.style["l10"]=1;
 			var xRemain=mxUtils.getValue(this.state.style,'remain',partWidth);
 			c.begin();
 			...
 			if (xRemain!=null) {
 				...
 				if (parseInt(tempInt)!==tempInt) {
 					tempInt=(parseInt(tempInt)+1)*NumberLine.prototype.cst.Interval;
 					value+=tempInt;
 				}else{
 					value+=(tempInt*NumberLine.prototype.cst.Interval);
 				}
 			}
 		}else{
 		...
 		for (var i = 0; i <=end; i+=step) {
 			if ((start-partWidth+i)<=end) {
 				...
 				c.setFontSize (15);
 				var tempValue=value;
 				if (parseInt(NumberLine.prototype.cst.Interval)!==NumberLine.prototype.cst.Interval) {
 					tempValue=tempValue.toFixed(1);
 				}
 				c.text(start+i,currentPositionY+15,0,0,tempValue.toString(),'center');
 				value+=NumberLine.prototype.cst.Interval;

 			}
 		}
 		c.stroke();
 		c.end();
 		var s=mxUtils.getValue(this.state.style,'start',w/30);
 		var e=mxUtils.getValue(this.state.style,'end',w-w/30);
 		c.begin();
 		c.setStrokeColor("red");
 		c.moveTo(s,h/3);
 		c.lineTo(s-2,h/5);
 	...
 	function createNumberLineHandles()
 	{
 		...
 		{	
 			...
 			handles.push(Graph.createHandle(state, ['radi'], function(bounds)
 			{
 				...
 				var start=mxUtils.getValue(this.state.style,'start',bounds.width/30);
 				return new mxPoint(bounds.x+start,bounds.y+bounds.height/3);
 			}, function(bounds, pt)
 			{	
 			...
 			handles.push(Graph.createHandle(state, ['radi'], function(bounds)
 			{
 				...
 				var end=mxUtils.getValue(this.state.style,'end',bounds.width-bounds.width/30);
 				return new mxPoint(bounds.x+end,bounds.y+bounds.height/3);
 			}, function(bounds, pt)
 			{	
	...
	Clock.prototype.paintBackground = function (c, x, y, w, h) {
		...
		c.moveTo(x6, y6);
		var x7, y7, x8, y8;
		x7 = r + (r2 - 3) * findX(mradi - 2);
		y7 = r - (r2 - 3) * findY(mradi - 2);
		x8 = r + (r2 - 3) * findX(mradi + 2);
		y8 = r - (r2 - 3) * findY(mradi + 2);
		c.lineTo(x7, y7);
		c.lineTo(x8, y8);
		c.lineTo(x6, y6);
		c.moveTo(x6, y6);
		...
		c.moveTo(x12, y12);
		var x13, y13, x14, y14;
		x13 = r + (r3 - 3) * findX(hradi - 2);
		y13 = r - (r3 - 3) * findY(hradi - 2);
		x14 = r + (r3 - 3) * findX(hradi + 2);
		y14 = r - (r3 - 3) * findY(hradi + 2);
		c.lineTo(x13, y13);
		c.lineTo(x14, y14);
		c.lineTo(x12, y12);
		c.moveTo(x12, y12);
