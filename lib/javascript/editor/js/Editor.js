 ...
 Editor = function(chromeless, themes, model, graph, editable)
 {
 	...
 	this.graph.addListener(mxEvent.CELLS_ADDED ,function(sender, evt){
		...
		if(parent.identity == "fraction-pieces"){
			...
			parent.remove(parent.children.length - 1);
			var centerX = patternShape.geometry.x + 10 + (patternShape.geometry.width  - 2 * 10)/2;
			var centerY = patternShape.geometry.y + 10 + patternShape.geometry.height / 2;
			var degree = 60;
			var r = (patternShape.geometry.width - 2 * 10) / 2;
			var temp = r * Math.sin(degree / 2) * 2;
			var pieces=new mxCell(null,new mxGeometry(0,0,80,160),'shape=CirclePieces;aspect=fixed;fillColor=#64AA50;perimeter=stepPerimeter;align=center;fixedSize=1;marginTop=20;points=[];resizable=0;');
			pieces.vertex=true;
			pieces.geometry.x =  centerX;// + r * Math.sin(Math.PI / 3);// -  (60/180) * r; 
			temp = Math.cos(degree) * r;
			pieces.geometry.y = patternShape.geometry.y + 10 + (r - Math.cos(Math.PI/2) * r);
