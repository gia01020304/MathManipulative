 ...
 */
 function getTileValue(plainValue){
 	var values = plainValue.match(/[0-9]+/g);
 	if(values.length < 2){
 		return values[0] * 1;
 	} else {
 ...
 Editor = function(chromeless, themes, model, graph, editable)
 {
	 ...
	 this.status = '';
	 //2019/07/28 gnguyen start add
	 var origIsValidDropTarget = this.graph.isValidDropTarget;
	 var origIsCellSelectable = this.graph.isCellSelectable;
	 this.graph.isCellSelectable = function (cell) {
	 	if (cell.identity === "cube-child") {
	 		return false;
	 	}
	 	return origIsCellSelectable.call(this, cell);
	 }
	 this.graph.isValidDropTarget=function(cell, cells, evt){
	 	if(cell.identity === "FractionTileParent") {
	 		return true;
	 	} else if(cell.identity === "cube-fraction"){
	 		return true;
	 	} else if(cell.identity === "fraction-pieces") {
	 		return true;
	 	}
	 	return false;
	 }
		 //2019/07/28 gnguyen end add	 
		 this.graph.addListener(mxEvent.CELLS_ADDED ,function(sender, evt){
		 	
		 	var parent = evt.properties.parent;
		 	var cell = evt.properties.cells[0];
		 	console.log(cell);
		 	if (parent.identity === "FractionTileParent") {
		 		var children = parent.children;
		 		var tileChildLengthTotal = 0;
		 		for (var i = 0; i < children.length; i++) {
		 			var childValue = getTileValue(children[i].value);
		 			tileChildLengthTotal += childValue;
		 			children[i].geometry.width = childValue * parent.geometry.width;
		 		}
		 		if (tileChildLengthTotal > 1) {
		 			parent.remove(children.length - 1);
		 		}
		 	}
		 	if(cell.identity === "cube-fraction" && parent.identity == "cube-fraction"){
		 		sender.getModel().beginUpdate();
			// new cell
			var cubeBoundWidth = parent.children[parent.children.length - 1].geometry.width;
			var cubeWidth = parent.children[parent.children.length - 1].children[0].geometry.width;
			var spaceWidth = cubeBoundWidth - cubeWidth;
			var addedCubeBound = parent.children[parent.children.length - 1];
			parent.remove(parent.children.length - 1);
			var parentChildLength = parent.children.length;
			for (; cell.children.length !== 0; ) {
				var preItemGeomatry = parent.children[parent.children.length - 1].geometry;
				preItemGeomatry.x = preItemGeomatry.x + preItemGeomatry.width - spaceWidth/2-2;
				cell.children[0].setGeometry(preItemGeomatry);
				parent.insert(cell.children[0], parent.children.length);
			}
			sender.getModel().endUpdate();
		}
		if (cell.identity==="NumberLinePro") {
			
		}
		//2019/07/13 gnguyen start add
		if (cell.identity==="Block3") {
			cell.geometry.x=cell.geometry.x+30;
		}
		//2019/07/13 gnguyen end add
		if(parent.identity == "fraction-pieces"){
			
			var patternShape = parent.children[0];
			parent.remove(parent.children.length - 1);
			var centerX =
			patternShape.geometry.x +
			10 +
			(patternShape.geometry.width - 2 * 10) / 2;
			var centerY =
			patternShape.geometry.y + 10 + patternShape.geometry.height / 2;
			var degree = 60;
			var r = (patternShape.geometry.width - 2 * 10) / 2;
			var temp = r * Math.sin(degree / 2) * 2;
			var pieces = new mxCell(
				null,
				new mxGeometry(0, 0, 80, 160),
				"shape=CirclePieces;aspect=fixed;fillColor=#64AA50;perimeter=stepPerimeter;align=center;fixedSize=1;marginTop=20;points=[];resizable=0;"
				);
			pieces.vertex = true;
			pieces.geometry.x = centerX; // + r * Math.sin(Math.PI / 3);// -  (60/180) * r;
			temp = Math.cos(degree) * r;
			pieces.geometry.y =
			patternShape.geometry.y + 10 + (r - Math.cos(Math.PI / 2) * r);
			pieces.geometry.height = patternShape.geometry.height - 20;
			parent.insert(pieces, 0);
			pieces.geometry.x = centerX + r * Math.sin(Math.PI / 2);
		}
		if(cell.identity === "integerCounter") {
			
		}
	});
		 this.graph.addListener(mxEvent.CELLS_RESIZED, function(sender, evt) {
		 	var cell = evt.properties.cells[0];
		 	if (cell.identity === "FractionTile") {
		 		var childrentCount = evt.properties.cells[0].getChildCount();
		 		for (var i = 0; i < childrentCount; i++) {
		 			var child = cell.getChildAt(i);
		 			var childValue = getTileValue(child.value);
		 			child.geometry.width = cell.geometry.width * childValue;
		 		}
		 	}
		 	if (cell.identity === "cube-fraction") {
		 		for (var i = 0; i < cell.children.length; i++) {
		 			cell.children[i].geometry.width =
		 			cell.geometry.width / cell.children.length;
		 		}
		 	}
		 	if(cell.identity === "algebra"){
		 		var fontSizeByParent = cell.geometry.width * 0.5;
		 		cell.value = cell.value.replace(/font-size:.+;/g, `font-size:${fontSizeByParent}px;`);
		 	}
		 });
		 ...
		 this.graph.addListener(mxEvent.LABEL_CHANGED, function(sender, evt) {
		 	...
		 	var iCheck=null;
		 	var lNumberLine = ["minimum", "interVal", "maximum"];
		 	switch (cell.identity) {
		 		case 'minimum':
		 		iCheck = parseFloat(cell.value);
		 		break;
		 		case 'interVal':
		 		iCheck = parseFloat(cell.value);
		 		if (iCheck!=null&&!isNaN(iCheck)&&iCheck<=0) {
		 			iCheck=null;
		 		}
		 		break;
		 		case 'maximum':
		 	...
		 	if (lNumberLine.includes(cell.identity)) {
		 		sender.getModel().beginUpdate();
		 		try {
		 			if (iCheck===null|| isNaN(iCheck)) {
		 				sender.model.setValue(cell,"");
		 				return false;
		 			}
		 			sender.model.setValue(cell,iCheck);
		 		} catch (e) {
		 		} finally {
		 			sender.getModel().endUpdate();
		 		}
		 ...
		 this.graph.addListener(mxEvent.CLICK, function(sender, evt) {
			 debugger;
		 	if(textAble === true) {
		 		$("#geDiagramContainer svg").css("cursor", "auto");
		 		sender.insertTextToolClick(evt.properties.event);
		 		textAble = false;
		 		$("#geDiagramContainer svg, #geDiagramContainer svg *").css("cursor", "");
		 	}
		 	...
		 	if (cell != null) {
		 		...
		 		if (cell.value === "Metric") {
		 			...
		 			var parentGeometry = parent.geometry;
		 			processTransferRuler(
		 				sender,
		 				"Standard",
		 				"rulerStandard",
		 				parentGeometry.width-60,
		 				parentGeometry.height,
		 				parentGeometry.x,
		 				parentGeometry.y,
		 				false
		 				);
		 			sender.removeCells([parent]);
		 		}
		 		if (cell.value === "Standard") {
		 			var parent = cell.parent;
		 			var parentGeometry = parent.geometry;
		 			processTransferRuler(
		 				sender,
			...
			//2016/06/15 gnguyen start add
			switch (cell.identity) {
				case "number-protractor":
				this.selectCellForEvent(cell);
				break;
				case "build":

				var parent = sender.getDefaultParent();
				
				var minimum = parseFloat(cell.parent.children[1].value);
				var interval = parseFloat(cell.parent.children[3].value);
				var maximum = parseFloat(cell.parent.children[5].value);
				sender.getModel().beginUpdate();
				try {
					if (minimum>=maximum){
						sender.model.setValue(cell.parent.children[1],"");
						sender.model.setValue(cell.parent.children[5],"");
						return false;
					}
				} catch (e) {
				} finally {
				...
				sender.getModel().beginUpdate();
				try {
					// var numberline = mxCellRenderer.prototype.getShape("numberline");
					// numberline.prototype.cst = {
					// 	Minimum: minimum,
					// 	Interval: interval,
					// 	Maximum: maximum
					// };
					// 
					debugger;
					var style="editable=0;resizable=0;aspect=fixed;shape=numberline";
					var total=Math.round((maximum-minimum)/interval)*45+40;
					cell.parent.parent.getGeometry().width = total;
					cell.parent.parent.setStyle(style+";Maximum="+maximum+";Minimum="+minimum+";Interval="+interval);
					sender.getView().clear(cell.parent.parent, false, false);
					sender.getView().validate();
				...
				var graph = sender;
					...
					var newValue = "";
					var h = cell.parent.geometry.height;
					var w = cell.parent.geometry.width;
					var cells = [cell.parent];
					if (newValue != null && (newValue.length > 0 || cells.length > 0)) {
						...
						try {
							...
							if(style.image === cell.image) {
								newValue = cell.imageOpposite;
							} else {
								newValue = cell.image;
							}
							graph.setCellStyles(
				...
				sender.getModel().beginUpdate();
				try {
					var parentGroup=cell.parent;
					if (parentGroup.style.includes("group")) {
						sender.removeCells([cell]);
						var listChild=parentGroup.children;
						listChild.forEach( function(element, index) {
							
							if (element.style.includes("group")) {
								element.geometry.y=element.geometry.y+index*10;
								element.geometry.x=element.geometry.x+index*30;
								var unGroup=new mxCell(null,new mxGeometry(0,0,30,30),'shape=image;html=1;editable=0;movable=0;vimageAspect=1;aspect=fixed;image='+pathRelative+'lib/images/un.png');
								unGroup.vertex=true;
								unGroup.geometry.relative = true;
								unGroup.geometry.offset = new mxPoint(-30, element.geometry.height);
								unGroup.identity="un-group";
								element.insert(unGroup);
							}else{
								element.geometry.y=element.geometry.y-index*10;
							}
						...
						this.selectCellForEvent(parentGroup);
						sender.ungroupCells();
				...
				try {
					var parent=cell.parent;
					sender.getModel().setVisible(parent, false);
					parent.children.forEach( function(element, index) {
						sender.getModel().setVisible(element, false);
					});
				} catch(e) {
				} finally {
					sender.refresh();
	...
	this.graph.addListener(mxEvent.CELLS_MOVED, function(sender, evt) {
		...
		console.log(cell);
		if (cell != null) {
			if (cell.identity === "number-protractor") {
				var point = cell.geometry.offset;
				if (point.y > 0) {
					cell.geometry.offset = new mxPoint(100, -10);
					cell.value = 0;
					return false;
				}
				var y = Math.abs(point.y) - 10;
				var x = cell.parent.geometry.width / 2 + (point.x + 25);
				var t1 = calcAngleDegrees(x, y);
				var t2 = Math.round(t1);
				cell.value = t2;
			}
			if (cell.identity === "hide-ruler") {
				cell.geometry.offset = new mxPoint(-30,-95);
				return false;
			}
		}
	});
 ...
 */
 Editor.prototype.readGraphState = function(node) {
	...
	this.graph.connectionArrowsEnabled = false; //node.getAttribute('arrows') != '0';
 ...
 */
 ...
 Editor.prototype.test = function(xml) {
 	this.graph.model.beginUpdate();

 	try {
 		this.graph.model.clear();
 		this.graph.view.scale = 1;
 		var wrapper = $("#geDiagramContainer svg");
 		wrapper.html(xml);
 	} finally {
 		this.graph.model.endUpdate();
 	}
 ...
 */
 (function() {
			...
			if (graph.pageVisible) {
				...
				if (firstChild != null) {
					...
					this.backgroundPageShape.scale = 1;
						...
						this.backgroundPageShape.node.id = "geBackgroundPage";
						this.backgroundPageShape.node.style.width = "unset";
	...
	mxGraphView.prototype.validateBackgroundStyles = function() {
		...
		var image = "none";
		var position = "";
		//sonvc change from graph.isGridEnabled()
		...
		if (graph.view.backgroundPageShape != null) {
			...
			graph.container.id = "geDiagramContainer";
