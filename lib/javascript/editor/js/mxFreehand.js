function mxFreehand(graph)
{
	...
	this.setEnabled = function(isEnabled)
	{
		...
		try {
			graph.allowAutoPanning=!isEnabled;
	...
	this.stopDrawing = function() 
	{
		if (partPathes.length > 0) 
		{
			...
			if (w > 0 && h > 0)
			{
		        ...
		        var style = mxConstants.STYLE_SHAPE + '=stencil(' + Graph.compress(drawShape) + ');fillColor=none;rotatable=0;editable=0;strokeWidth=2;strokeColor='+ graph.currentVertexStyle["STYLE_STROKECOLORPen"];
    ...
    mxEvent.addGestureListeners(svgElement, function (e) 
    {
    	...
    	path.setAttribute("stroke", graph.currentVertexStyle["STYLE_STROKECOLORPen"]  || "#000");
    	...
    	path.setAttribute("class","pen-tool");
