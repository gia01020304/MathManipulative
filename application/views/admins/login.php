<?php if(isset($isSaveSuccessful)){ ?>
   <?php if($isSaveSuccessful === false) { ?>
    <div class="alert alert-danger" role="alert" id="msgerror">User name or password is not correct!</div>
   <?php } else { ?>
    <div class="alert alert-success" role="alert" id="msgerror">Save config successfully!</div>
  <?php  }
  }
?>
<div id="frmLogin" class="container-fluid" style="margin-left:80px;margin-right:80px;">
	<form id="frmlogin" method="post" action="<?=base_url()?>/AuthenticateCL/adminLogin">
