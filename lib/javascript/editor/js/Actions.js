 ...
 */
 Actions.prototype.init = function()
 {
	...
	// }, null, null, Editor.ctrlKey + '+Mdsad');
	...
	//giamod
	this.addAction('eraser',(elt)=>{
		$('path[class="pen-tool"]').remove();
	}, null, null, 'eraser')

	this.addAction('pen-tool',(elt)=>{
		this.editorUi.pickColor($("#pen-tool").attr('color'), function(newColor)
		{
			...
			img.src=pathRelative+'/lib/images/pencil.png';
			...
			var svg = d3.select("#geDiagramContainer svg")
			.call(d3.drag()
			...
			function  dragEnd() {
				console.log('end');
				var drawPoints=d3.event.subject;
				var maxX = drawPoints[0][0], minX = drawPoints[0][0], maxY = drawPoints[0][1], minY = drawPoints[0][1];

				for (var i = 1; i < drawPoints.length; i++) 
				{
					maxX = Math.max(maxX, drawPoints[i][0]);
					minX = Math.min(minX, drawPoints[i][0]);
		        ...
		        var drawShape = '<shape strokewidth="inherit" strokeColor="red"><foreground><path><move x="'+ drawPoints[0].x.toFixed(2) + '" y="' + drawPoints[0].y.toFixed(2) + '"/>';
		        ...
		        var cell = new mxCell('', new mxGeometry(minX / s - tr.x, minY / s - tr.y, w / s, h / s), style+";strokeColor="+newColor+";strokeWidth=2");
		        cell.vertex = 1;
...
this.addAction('hand-tool',(elt)=>{
	drawFree=false;
	line=null;
	$('#geDiagramContainer svg').css('cursor', "");
}, null, null, 'hand-tool')
this.addAction('delete-background',(elt)=>{
	var editor = this.editorUi;
	editor.setBackgroundImage(null);
}, null, null, 'delete-background')

this.addAction('ruler',(elt)=>{

	var temp=graph.getPageSize();
	var oneMm=Math.floor(temp.width/210);
	var widthRuler=(oneMm*25)*6;
	processTransferRuler(graph,"Metric","rulerMetric",widthRuler,100,null,null,true);
}, null, null, 'Ruler')
	// 2019/06/10 gnguyen start add

	this.addAction('protractor',(elt)=>{
		var cell=null;
		if (typeof graph.idProtractor!=="undefined"&&graph.idProtractor!==null) {
			cell=graph.getModel().getCell(graph.idProtractor);
			if (typeof cell!=="undefined"&&cell!=null) {
				graph.getModel().beginUpdate();
				try {
					graph.getModel().setVisible(cell, !cell.visible);
					cell.children.forEach( function(element, index) {
						graph.getModel().setVisible(element, !element.visible);
					});
				} catch(e) {
				} finally {
					graph.getModel().endUpdate();
		...
		try {
			var pt = graph.getFreeInsertPoint();
			var cell = graph.insertVertex(parent, null, null, pt.x,pt.y , 400, 275, 'editable=0;resizable=0;aspect=fixed;shape=protractor');
			cell.identity='protractor';
			var symbol2 = new mxCell('', new mxGeometry(0.95,0,20,20),'editable=0;resizable=0;aspect=fixed;rotationEnabled=0;constituent=1;');
			symbol2.value="X";
			symbol2.identity = "hide-ruler";
			symbol2.connectable=false;
			symbol2.geometry.relative = true;
			symbol2.vertex = true;
			graph.idProtractor=cell.id;
			cell.insert(symbol2);
		} catch(e) {
		}finally {
			graph.getModel().endUpdate();
	...
	}, null, null, 'protractor')

	// 2019/06/10 gnguyen end add
	
	//2019/06/15 gnguyen start add
	//2019/07/04 gnguyen start del
	// this.addAction('clock',(elt)=>{

	// 	new mxRubberband(graph);
	// 	var parent = graph.getDefaultParent();
	// 	graph.getModel().beginUpdate();
	// 	try {
	// 		var v4 = graph.insertVertex(parent, null,null, 100, 100, 300, 300, 'aspect=fixed;shape=clock;');
	// 		// var v4 = graph.insertVertex(parent, null,null, 100, 0, 50, 300, 'shape=clock;');

	// 	} finally
	// 	{
	// 		graph.getModel().endUpdate();
	// 	}
	// 	// processClock(graph,"Clock",300,300,20,20);
	// }, null, null, 'clock')
	//2019/07/04 gnguyen end del
	// this.addAction('QUICKSCROLL',(elt)=>{

	// 	new mxRubberband(graph);
	// 	var parent = graph.getDefaultParent();
	// 	graph.getModel().beginUpdate();
	// 	try {
	// 		var v4 = graph.insertVertex(parent, null,null, 100, 100, 50, 300, 'aspect=fixed;shape=QUICKSCROLL;');
	// 		// var v4 = graph.insertVertex(parent, null,null, 100, 0, 50, 300, 'shape=clock;');
	...
	this.addAction('showerror',(elt)=>{

	}, null, null, 'showerror')
	this.addAction('new-file',(elt)=>{
		window.open(pathRelative,'_blank');
	}, null, null, 'new-file')
	this.addAction('share',(elt)=>{
		addFeature(3);
		if ($('#user').length==0) {
	...
	this.addAction('save-logged',(elt)=>{
 ...
 */
 ...
 function processTransferRuler(graph,value,typeRuler,width,height,x,y,oldObject) {

 	var parent = graph.getDefaultParent();
 	graph.getModel().beginUpdate();
 	var cell=null;
 	try {
 		if (typeof graph.idRuler!=="undefined"&&graph.idRuler!==null&&oldObject==true) {
 			cell=graph.getModel().getCell(graph.idRuler);
 			if (typeof cell!=="undefined"&&cell!=null) {
 				graph.getModel().beginUpdate();
 				try {
 					graph.getModel().setVisible(cell, !cell.visible);
 		...
 		var cell = graph.insertVertex(parent, null, null, x,y, width+40+20, 100, 'rotationEnabled=0;editable=0;resizable=0;aspect=fixed;shape='+typeRuler);
 		cell.identity='ruler';
 		cell.setConnectable(false);
 		var symbol2 = new mxCell('', new mxGeometry(0.93,0.1,20,20),'editable=0;resizable=0;aspect=fixed;rotationEnabled=0;');
 		symbol2.value="X";
 		symbol2.identity = "hide-ruler";
 		symbol2.connectable=false;
 		symbol2.geometry.relative = true;
 		...
 		var symbol = new mxCell('', new mxGeometry(1,1,50,20),'editable=0;resizable=0;aspect=fixed;rotationEnabled=0;');
 		symbol.value=value;
 		symbol.identity="ruler-"+typeRuler;
 		symbol.vertex = true;			
 		symbol.geometry.relative = true;
 		symbol.geometry.offset = new mxPoint(-55, -27);

 		cell.connectable=false;
 		graph.idRuler=cell.id;
 		cell.insert(symbol2);
 		cell.insert(symbol);
 	} catch(e) {
 	}finally {
 	...
 	// 	if (typeof graph.idRuler!=="undefined"&&graph.idRuler!==null&&oldObject==true) {
 	// 		var cell=graph.getModel().getCell(graph.idRuler);
 	// 		if (typeof cell!=="undefined"&&cell!=null) {
 	// 			graph.getModel().setVisible(cell, true);
 	// 			cell.children.forEach( function(element, index) {
 	// 				graph.getModel().setVisible(element, true);
 	// 			});
 	// 			return false;
 	// 		}
 	// 	}
 	...
 	// 	var v1 = graph.insertVertex(parent, null, null, x, y, width, height, 'editable=0;resizable=0;aspect=fixed;shape='+typeRuler);
 	...
 	// 	symbol.geometry.relative = true;
 	// 	symbol.geometry.offset = new mxPoint(-55, -27);
 	// 	var symbol2 = new mxCell('', new mxGeometry(1,1,20,20),'editable=0;resizable=0;aspect=fixed;rotationEnabled=0;constituent=1;');
 	// 	symbol2.value="X";
 	// 	symbol2.identity = "hide-ruler";
 	// 	symbol2.connectable=false;
 	// 	symbol2.geometry.relative = true;
 	// 	symbol2.vertex = true;
 	// 	symbol2.geometry.offset = new mxPoint(-30,-height+5);
 ...
 //2019/06/16 gnguyen start add
 function processClock(graph,nameShape,width,height,x,y){
 	var parent = graph.getDefaultParent();
 	graph.getModel().beginUpdate();
 	try {
 		var v1 = graph.insertVertex(parent, null, null, x, y, width, height, 'aspect=fixed;shape='+nameShape);
 	} catch(e) {
 	}finally {
 		graph.getModel().endUpdate();
 	}
 }
 //2019/06/16 gnguyen end add
 //2016/06/15 gnguyen start add
 function processProtractor(graph,nameShape,width,height,x,y) {
 	var parent = graph.getDefaultParent();
 	graph.getModel().beginUpdate();
 	graph.disconnectOnMove = false;
 	try {
 		var v1 = graph.insertVertex(parent, null, null, x, y, width, height, 'shape='+nameShape);

 		var number = new mxCell('', new mxGeometry(1,1,50,20));
 		number.value="0";
 		number.identity="number-protractor";
 		number.vertex = true;
 		number.connectable=false;
 		number.geometry.relative = true;
 		number.geometry.offset = new mxPoint(100,-10);
 		number.style="resizable=0;constituent=1;";
 		var v11 = graph.insertVertex(v1, null, '',0.5, 1, 0, 0,
 			'id=temp;portConstraint=northsouth;', true);

 		number.rotationEnabled = false;
 		graph.addCell(number,v1);
 		var vertexHandlerUnion = mxVertexHandler.prototype.union;

 		graph.insertEdge(parent, null, '', v1.getChildAt(0),number,"startArrow=oval;strokeWidth=4;strokeColor=#69b630;resizable=0;");
 	} catch(e) {
 	}finally {
 		graph.getModel().endUpdate();
 	}
