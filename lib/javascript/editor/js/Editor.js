 ...
 */
 ...
 Editor = function(chromeless, themes, model, graph, editable)
 {
 	...
 	this.graph.addListener(mxEvent.CELLS_ADDED ,function(sender, evt){
 		...
 		if(parent.value === "Fraction Tile Container"){
 		...
 		if(cell.identity === "cube-fraction" && parent.identity == "cube-fraction"){
			...
			for (; cell.children.length !== 0; ) {
				parent.insert(cell.children[0], parent.children.length - 1);
			}
			// for(var i = 0;i<parent.children.length;i++){
			// 	parent.children[i].geometry.width = parent.geometry.width/parent.children.length;
			// }
		}
	...
	this.graph.addListener(mxEvent.CLICK, function(sender, evt) {
		...
		if (cell != null) {
			if (cell.value === "Metric") {
				...
				registryShape(
					pathRelative + "/lib/stencils/ruler.xml",
					"ruler-standard"
				);
			...
			if (cell.value === "Standard") {
				var parent = cell.parent;
				var parentGeometry = parent.geometry;
				registryShape(pathRelative + "/lib/stencils/ruler.xml", "ruler-metric");
				processTransferRuler(
					sender,
					"Metric",
					"Ruler Metric",
			...
			switch (cell.identity) {
				case "number-protractor":
					...
					break;
				case "build":
					var minimum = TryParseInt(
						cell.parent.parent.children[1].children[1].value,
						0
					);
					var interval = TryParseInt(
						cell.parent.parent.children[1].children[3].value,
						1
					);
					var maximum = TryParseInt(
						cell.parent.parent.children[1].children[5].value,
						1
					);

					minimum = minimum < -10 ? -10 : minimum;
					maximum = maximum > 10 ? 10 : maximum;

					var parent = sender.getDefaultParent();
					sender.getModel().beginUpdate();
					try {
						sender.model.setValue(
							cell.parent.parent.children[1].children[1],
							minimum
						);
						sender.model.setValue(
							cell.parent.parent.children[1].children[3],
							interval
						);
						sender.model.setValue(
							cell.parent.parent.children[1].children[5],
							maximum
						);
						...
						sender.removeCells([cell.parent.parent.children[0]]);
						var numberLine = new mxCell(
							null,
							new mxGeometry(0, 0, 600, 100),
							"shape=numberline;"
						);
						numberLine.vertex = true;
						cell.parent.parent.insert(numberLine, 0);
					} catch (e) {
					} finally {
				...
				case "TurnMoney":
					...
					if (newValue != null && (newValue.length > 0 || cells.length > 0)) {
						...
						try {
							...
							// 	{
							// 		var cell1 = cells[0];
							// 		var geo = graph.getModel().getGeometry(cell1);
							...
							// 			geo.width = w;
							// 			geo.height = h;
							// 			graph.getModel().setGeometry(cell1, geo);
							// 		}
							// 	}
							// }
						} finally {
 	...
 	this.graph.addListener(mxEvent.CLICK, function (sender, evt) {
 		...
 		if (cell != null) {
 			...
 			if (cell.value==="Metric") {
 				var parent=cell.parent;
 				var parentGeometry=parent.geometry;
 				registryShape(pathRelative+"/lib/stencils/ruler.xml","ruler-standard");
 				processTransferRuler(sender,"Standard","Ruler Standard",parentGeometry.width,parentGeometry.height,parentGeometry.x,parentGeometry.y);
 				sender.removeCells([parent]);
 			}
 			if (cell.value==="Standard") {
 				var parent=cell.parent;
 				var parentGeometry=parent.geometry
 				registryShape(pathRelative+"/lib/stencils/ruler.xml","ruler-metric");
 				processTransferRuler(sender,"Metric","Ruler Metric",parentGeometry.width,parentGeometry.height,parentGeometry.x,parentGeometry.y);
 				sender.removeCells([parent]);
 			}
 			//2016/06/15 gnguyen start add
 			switch (cell.identity) {
 				...
 				var minimum=TryParseInt(cell.parent.parent.children[1].children[1].value,0);
 				var interval=TryParseInt(cell.parent.parent.children[1].children[3].value,1);
 				var maximum=TryParseInt(cell.parent.parent.children[1].children[5].value,1);
 				...
 				sender.getModel().beginUpdate();
 				try {
 					sender.model.setValue(cell.parent.parent.children[1].children[1], minimum);
 					sender.model.setValue(cell.parent.parent.children[1].children[3], interval);
 					sender.model.setValue(cell.parent.parent.children[1].children[5], maximum);
 					...
 					sender.removeCells([cell.parent.parent.children[0]]);
 					var numberLine=new mxCell(null,new mxGeometry(0,0,600,100),'shape=numberline;');
 					numberLine.vertex=true;
 					cell.parent.parent.insert(numberLine,0);
