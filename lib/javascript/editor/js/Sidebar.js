 ...
 Sidebar.prototype.addMoneyPalette = function(id, title, prefix, postfix, items, titles, tags)
 {
 	...
 	for (var i = 0; i < items.length; i++)
 	{
 		(mxUtils.bind(this, function(item, title, tmpTags)
 		{
 			...
 			fns.push(this.createVertexTemplateEntry('image;html=1;editable=0;labelBackgroundColor=#ffffff;image=' + prefix + item + postfix,
 				this.defaultImageWidth * 3, this.defaultImageHeight * 3, '', title, title != null, null, this.filterTags(tmpTags), false, "money",prefix + item + postfix, prefix + imageOpposite + postfix));
 		}))(items[i], (titles != null) ? titles[i] : null, (tags != null) ? tags[items[i]] : null);
 	}
