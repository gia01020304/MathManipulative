 ...
 */
 Toolbar.prototype.init = function()
 {
	...
	if (sw >= 320)
	{
		var elts = this.addItems(['new-file']);
		var node=document.createElement("div");
		node.className='fas fa-file'
		elts[0].childNodes[0].classList.remove('geSprite');
		elts[0].childNodes[0].appendChild(node);
		elts[0].style.width = 'unset';
		elts[0].setAttribute('title','New');
	}
	if (sw >= 320)
	{
		var elts = this.addItems(['save-logged',"-"]);
		var node=document.createElement("div");
		node.className='far fa-save';
		if (typeof isAuthen==="undefined"|| isAuthen==false) {
		...
		elts[0].childNodes[0].appendChild(node);
		elts[0].style.width = 'unset';
		elts[0].setAttribute('title','Save');
	}
	//giaendmod
	...
	if (sw >= 320)
	{
		var elts = this.addItems(['delete-background']);
		var node=document.createElement("i");
		node.className="fas fa-calendar-times";
		elts[0].childNodes[0].classList.remove('geSprite');
		elts[0].childNodes[0].appendChild(node);
		elts[0].style.width = 'unset';
		elts[0].setAttribute('title','Remove background');
	}
	...
	if (sw >= 320)
	{
		var elts = this.addItems(["-",'pen-tool']);
		var node=document.createElement("i");
		node.className="fas fa-pen";
		elts[1].childNodes[0].classList.remove('geSprite');
		elts[1].setAttribute('id', 'pen-tool');
		elts[1].setAttribute('color', '#000000');
		elts[1].childNodes[0].appendChild(node);
		elts[1].style.width = 'unset';
		elts[1].setAttribute('title','Pen tool');
	}
	if (sw >= 320)
	{
		var elts = this.addItems(['eraser']);
		var node=document.createElement("i");
		node.className="fas fa-eraser";
		elts[0].childNodes[0].classList.remove('geSprite');
		elts[0].setAttribute('id', 'eraser');
		elts[0].setAttribute('color', '#000000');
		elts[0].childNodes[0].appendChild(node);
		elts[0].style.width = 'unset';
		elts[0].setAttribute('title','Eraser paint');
	}
	if (sw >= 320)
	{
		var elts = this.addItems(['hand-tool',"-"]);
		var node=document.createElement("i");
		node.className="far fa-hand-paper";
		elts[0].childNodes[0].classList.remove('geSprite');
		elts[0].childNodes[0].appendChild(node);
		elts[0].style.width = 'unset';
		elts[0].setAttribute('title','Hand tool');
	}
	if (sw >= 320)
	{
		debugger;
		var elts = this.addItems(['ruler']);
		var node=document.createElement("i");
		node.className="fa fa-ruler";
		elts[0].childNodes[0].classList.remove('geSprite');
		elts[0].childNodes[0].appendChild(node);
		elts[0].style.width = 'unset';
		elts[0].setAttribute('title','Ruler');

		var elts = this.addItems(['protractor',"-"]);
		var node=document.createElement("i");
		node.innerHTML="<img src=\""+pathRelative+"lib/images/protractor.png\" alt=\"Protractor\">";
		elts[0].childNodes[0].classList.remove('geSprite');
		elts[0].childNodes[0].appendChild(node);
		elts[0].style.width = 'unset';
		elts[0].setAttribute('title','Protractor');
	}
	if(sw >= 320){
		var elts = this.addItems(['group']);
		var node=document.createElement("i");
		node.className="fas fa-object-group";
		elts[0].childNodes[0].classList.remove('geSprite');
		elts[0].childNodes[0].appendChild(node);
		elts[0].style.width = 'unset';
		elts[0].setAttribute('title','Group');

		var elts = this.addItems(['ungroup']);
		var node=document.createElement("i");
		node.className="fas fa-object-ungroup";
		elts[0].childNodes[0].classList.remove('geSprite');
		elts[0].childNodes[0].appendChild(node);
		elts[0].style.width = 'unset';
		elts[0].setAttribute('title','UnGroup');
	}

	// if (sw >= 320)
	// {
	// 	var elts = this.addItems(['clock']);
	// 	var node=document.createElement("i");
	// 	node.innerHTML="<img src=\""+pathRelative+"lib/images/clock.png\" alt=\"Protractor\">";
	// 	elts[0].childNodes[0].classList.remove('geSprite');
	// 	elts[0].childNodes[0].appendChild(node);
	// 	elts[0].style.width = 'unset';
	// 	elts[0].setAttribute('title','Clock');
	// }
	//2019/07/04 gnguyen end del
	
	// if (sw >= 320)
	// {
	// 	var elts = this.addItems(['QUICKSCROLL']);
	// 	var node=document.createElement("i");
	// 	node.innerHTML="<img src=\""+pathRelative+"lib/images/clock.png\" alt=\"Protractor\">";
	// 	elts[0].childNodes[0].classList.remove('geSprite');
	// 	elts[0].childNodes[0].appendChild(node);
	// 	elts[0].style.width = 'unset';
	// 	elts[0].setAttribute('title','QUICKSCROLL');
	// }
	...
	if (sw >= 320)
	{
		var elts = this.addItems(['-','share','-']);
		var node=document.createElement("i");
		node.className='fas fa-share-alt';
		...
		elts[1].id='btnShare';
		elts[1].setAttribute('title','Share');
	}
	//giaendmod
	if (sw >= 320)
	{
		var elts = this.addItems(['showerror']);
		var node=document.createElement("div");
		node.id='divmsgparent';
		elts[0].childNodes[0].classList.remove('geSprite');
		elts[0].childNodes[0].appendChild(node);
		elts[0].style.width = 'unset';
		elts[0].setAttribute('title','');
		elts[0].cursor = 'unset';
	}
	//giaendmod
};
